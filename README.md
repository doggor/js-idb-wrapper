...
Get store's keypath:
```js
IDB('app_db').store('users').key(function(keyPath) {
...
//add data
users.add({name: "Andy", email: "andy@mail.com"});  //return promise

//update data with key(id) = 1
users.update({id: 0, name: "Ben", email: "ben@mail.com"});  //return promise

//remove data with key = 1
users.delete(0);  //return promise

//remove all data
users.clear();  //return promise
```
...
query01.first(function(item, key) {/*...*/});  //return promise

//iterate each item
query03.each(function(item, key) {/*...*/});  //return promise

//get the whole result set
query05.list(function(itemArray, keyArray) {/*...*/});  //return promise

//count the number of items in the result set
query07.count(function(totalNumber) {/*...*/});  //return promise
...
```js
IDB('app_db').remove();  //return promise
```
...
});  //return the database reference for chaining
```
